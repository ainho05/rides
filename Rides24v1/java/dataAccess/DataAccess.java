...
public boolean hasPassengerTraveledWithDriver(String passengerEmail, String driverEmail) {
    ...
    // IT3 (he cambiado este método)
    public List<Driver> getDriversForPassenger(String passengerEmail) {
    	
    	EntityManager em = emf.createEntityManager(); // Abre un nuevo EntityManager
        ...
        try {
            ...
            TypedQuery<Driver> query = em.createQuery(queryStr, Driver.class);
            query.setParameter("passengerEmail", passengerEmail);
            drivers = query.getResultList();
            return drivers;
        } finally {
            if (em.isOpen()) {
                em.close(); // Cierra el EntityManager después de la consulta
            }
        }
 ...
 // IT3 
    public boolean saveMonedero(Monedero monedero) {
        try {
            db.getTransaction().begin();
            db.persist(monedero);
            db.getTransaction().commit();
    ...
    public Monedero getMonederoByUserEmail(String userEmail) {
        Monedero monedero = null;
        EntityManager em = null;
        try {
        	em = emf.createEntityManager();
            TypedQuery<Monedero> query = em.createQuery(
                    "SELECT m FROM Monedero m WHERE m.userEmail = :userEmail", Monedero.class);
            query.setParameter("userEmail", userEmail);
            List<Monedero> monederos = query.getResultList(); // Cambia a getResultList()
            if (monederos != null && !monederos.isEmpty()) {
                monedero = monederos.get(0); // Obtiene el primer monedero de la lista
            }
            
	    } catch (NoResultException e) {
	        // Manejar el caso en que no se encuentra el monedero para el usuario dado
	        return null; // o lanzar una excepción personalizada si prefieres
	    } catch (Exception e) {
	        e.printStackTrace(); // Log the exception
	        // Considerar lanzar una excepción personalizada aquí para indicar un error de acceso a datos
	        throw new RuntimeException("Error al obtener el monedero del usuario", e);
	    } finally {
	    	if (em != null && em.isOpen()) // Verifica que el EntityManager esté abierto antes de cerrarlo
	            em.close();
    ...
    public boolean updateMonedero(Monedero monedero) {
    	// IT3
        ...
        try {
            ...
            if (em != null && em.isOpen()) {
                em.close(); // Cierra el EntityManager creado en este método
            }
        }
    }
    ...
    public void simularCargarSaldoConTarjeta(String userEmail, float cantidad, String numeroTarjeta, String cvc,
			String titular) {
        System.out.println(">> DataAccess: simularCargarSaldoConTarjeta => Usuario ID: " + userEmail +
                           ", Cantidad: " + cantidad +
                           ", Tarjeta: ****-****-****-" + numeroTarjeta.substring(numeroTarjeta.length() - 4));
        System.out.println("   Simulando transacción exitosa para la tarjeta.");

        EntityManager em = emf.createEntityManager();
        
        try {
        	em.getTransaction().begin();
         // Consulta JPA para buscar el Monedero por userEmail
            TypedQuery<Monedero> query = em.createQuery("SELECT m FROM Monedero m WHERE m.userEmail = :userEmail", Monedero.class);
            query.setParameter("userEmail", userEmail);
            List<Monedero> monederos = query.getResultList(); // Puede haber más de uno si no has definido userEmail como UNIQUE
            Monedero monedero = monederos.isEmpty() ? null : monederos.get(0); // Obtener el primero, o null si no hay ninguno


            if (monedero != null) {
                monedero.anadirSaldo(cantidad);
                em.merge(monedero); // Actualizar el monedero en la base de datos
                System.out.println("   Saldo de usuario " + userEmail + " actualizado a: " + monedero.getSaldo());
            } else {
                System.out.println("   No se encontró el monedero para el usuario: " + userEmail);
            }
            em.getTransaction().commit();
        } catch (Exception e) {
            if (em.getTransaction().isActive()) {
            ...
            e.printStackTrace();
            System.err.println("   Error al actualizar el saldo del monedero.");
        } finally {
        	if (em != null && em.isOpen()) {
                em.close(); // Cierra el EntityManager local
            }
        }
    }
    ...
    public Booking getBookingById(int bookingId) {
    	Booking booking = null;
        try {
            booking = db.find(Booking.class, bookingId);
        } catch (Exception e) {
            e.printStackTrace(); // Manejo de errores: ¡No lo dejes vacío!
        } finally {
            db.close();
        }
        return booking;
    ...
    public Booking getBookingByPassengerAndDriver(String driverEmail, String passengerEmail) {
        ...
        try {
            TypedQuery<Booking> query = db.createQuery(
                "SELECT b FROM Booking b WHERE b.ride.driver.email = :driverEmail AND b.user.email = :passengerEmail", Booking.class);
            query.setParameter("driverEmail", driverEmail);
            query.setParameter("passengerEmail", passengerEmail);
            List<Booking> results = query.getResultList();
            if (!results.isEmpty()) {
    ...
    public List<Booking> getBookingsByUserAndStatus(String userEmail, BookingStatus status) {
        EntityManager em = null;
    	try {
            em = emf.createEntityManager();
            return em.createQuery("SELECT b FROM Booking b WHERE b.user.email = :email AND b.status = :status", Booking.class)
    ...
    public boolean updateBookingStatus(Long bookingId, BookingStatus newStatus) {
        ...
        try {
            em = emf.createEntityManager();
            em.getTransaction().begin();
            int updatedCount = em.createQuery("UPDATE Booking b SET b.status = :status WHERE b.bookingId = :id")
