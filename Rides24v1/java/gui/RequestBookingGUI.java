...
public class RequestBookingGUI extends JFrame {
    ...
    public RequestBookingGUI(String passengerEmail, Ride selectedRide) {
        ...
        btnConfirm.addActionListener(e -> {
            ...
            // IT3
            double originalPrice = selectedRide.getPrice();
            double newPrice = originalPrice * numSeats; // Calcula el nuevo precio
            // Confirmación adicional del precio (opcional, pero recomendable)
            int confirmResult = JOptionPane.showConfirmDialog(
                    this,
                    "The total price for " + numSeats + " seats will be: " + String.format("%.2f", newPrice) + "€\nDo you want to confirm?",
                    "Confirm Booking",
                    JOptionPane.YES_NO_OPTION
            );
            if (confirmResult == JOptionPane.YES_OPTION) {
	            boolean resquestSent = facade.requestBooking(passengerEmail, selectedRide.getRideNumber(), numSeats, newPrice);
	            if (resquestSent) {
    ...
    private void reserveRide() {
        ...
        int numSeats=(Integer) comboBox.getSelectedItem();
        // IT3
        Ride ride = facade.getRide(rideId);
        
        if (ride == null) {
            JOptionPane.showMessageDialog(this, "Error: Could not retrieve ride details", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        double unitPrice = ride.getPrice();

        boolean success = facade.requestBooking(passengerEmail, rideId, numSeats, unitPrice);
        // hasta aquí
        if (success) {
