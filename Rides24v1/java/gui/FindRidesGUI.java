...
public class FindRidesGUI extends JFrame {
	...
	public FindRidesGUI(String userEmail)
	{
		...
		// IT 3: Componentes para filtrar por precio máximo
				maxPriceLabel = new JLabel("Precio máximo:");
				maxPriceLabel.setBounds(new Rectangle(10, 148, 100, 20)); // Ajusta la posición según tu layout
				getContentPane().add(maxPriceLabel);

				priceTextField = new JTextField(10);
				priceTextField.setBounds(new Rectangle(107, 149, 80, 20)); // Ajusta la posición
				getContentPane().add(priceTextField);

				JButton searchByPriceButton = new JButton("Filtrar por precio");
				searchByPriceButton.setBounds(new Rectangle(20, 178, 130, 25)); // Ajusta la posición
				getContentPane().add(searchByPriceButton);
				
				searchByPriceButton.addActionListener(new ActionListener() {
					@Override
					public void actionPerformed(ActionEvent e) {
						String maxPriceText = priceTextField.getText();
						try {
							double maxPrice = Double.parseDouble(maxPriceText);
							BLFacade blFacade = MainGUI.getBusinessLogic();
							List<Ride> allRides = blFacade.getAllAvailableRides(); // Obtener todos los viajes
							List<Ride> filteredRides = allRides.stream()
									.filter(ride -> ride.getPrice() <= maxPrice)
									.collect(java.util.stream.Collectors.toList());

							// Actualizar la tabla con los viajes filtrados
							tableModelRides.setRowCount(0); // Limpiar la tabla
							for (Ride ride : filteredRides) {
								Vector<Object> row = new Vector<>();
								row.add(ride.getDriver().getName());
								row.add(ride.getnPlaces());
								row.add(ride.getPrice());
								row.add(ride); // Para mantener la referencia al objeto Ride
								tableModelRides.addRow(row);
							}
							actualizarTablaConOrdenActual(); // Mantener el orden si está seleccionado
						} catch (NumberFormatException ex) {
							JOptionPane.showMessageDialog(FindRidesGUI.this, "Por favor, introduce un precio válido.", "Error", JOptionPane.ERROR_MESSAGE);
						}
					}
				});
